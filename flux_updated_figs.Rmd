...
I'm not sure about my values for these flux calculations... but basically increasing methane flux >> steady-state

```{r, echo=FALSE}
figureS1flux # 5.48 x 4.61
```
Figure S1. Methane fluxes across each core, integrated from modeled AOM rates derived from data at the time of sampling. Cores are colored by methane stage. PC1029 was omitted due to high uncertainty in modeling AOM rates (explain more?)

```{r, echo=FALSE}
figureS3ratemcrAplot # 6.43 x 4.28
```

Figure S2. Regression of mcrA gene copy numbers (per gram bulk sediment) to modeled AOM rates shows a linear relationship across samples from all cores (log-log transformation). Samples are colored according to stages of methane dynamics, and those that did not contain detectable mcrA were omitted. 

```{r, echo=FALSE}
figureS3ssjust # 6.43 x 4.28
```
...
In this table below, I realize that the total-core flux values do not match those that I showed in the first supplemental figure... so I must have made a mistake?
