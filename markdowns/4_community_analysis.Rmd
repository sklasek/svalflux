...
ps.frdp.seep <- subset_samples(ps.frdp, sample_data(ps.frdp)$core=="PC1029") # subset all PC1029 samples
ps.frdp.seep.ra <- transform_sample_counts(ps.frdp.seep, function(OTU) OTU/sum(OTU)) # relative abundances
ps.sulfurovum <- subset_taxa(ps.frdp.seep.ra, Genus=="Sulfurovum")
plot_bar(ps.sulfurovum, "depth", "Abundance", "OTU") # ASV5 is most abundant throughout core, but ASVs 31 and 105 are most abundant in the top two samples (1-2 cm). ASV72 also abundant, all others are very rare.

# prints the sequences of the dominant Sulfurovum ASVs in PC1029:
as.character(refseq(ps.frdp.seep)["ASV5"]) 
as.character(refseq(ps.frdp.seep)["ASV31"])
as.character(refseq(ps.frdp.seep)["ASV105"])
as.character(refseq(ps.frdp.seep)["ASV72"])
```

All of these ASVs have the same BLAST best match: Sulfurovum lithotrophicum 42BKT, from Inagaki et al 2004.
