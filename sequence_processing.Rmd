...
Reload ps.frdp and add more metadata to the sample data: SMT depth, peakAOM depth, distance above/below peakAOM depth, and classification of samples as above or below SMT
```{r}
library(phyloseq)
...
sample_data(ps.frdp)[which(sample_data(ps.frdp)$core=="GC1045"),16] <- 82 # assign SMT depths by core
sample_data(ps.frdp)[which(sample_data(ps.frdp)$core=="GC1048"),16] <- 320
sample_data(ps.frdp)[which(sample_data(ps.frdp)$core=="GC1068"),16] <- 108
sample_data(ps.frdp)[which(sample_data(ps.frdp)$core=="GC1069"),16] <- 138
sample_data(ps.frdp)[which(sample_data(ps.frdp)$core=="GC1070"),16] <- 69
sample_data(ps.frdp)[which(sample_data(ps.frdp)$core=="GC1081"),16] <- 56
sample_data(ps.frdp)$peakaom <- NA # creates a new metada column for peak AOM depth
sample_data(ps.frdp)[which(sample_data(ps.frdp)$core=="PC1029"),17] <- 45.3 # assign peak AOM depths by core
sample_data(ps.frdp)[which(sample_data(ps.frdp)$core=="GC1045"),17] <- 65
sample_data(ps.frdp)[which(sample_data(ps.frdp)$core=="GC1048"),17] <- 313.5
sample_data(ps.frdp)[which(sample_data(ps.frdp)$core=="GC1068"),17] <- 108
sample_data(ps.frdp)[which(sample_data(ps.frdp)$core=="GC1069"),17] <- 137
sample_data(ps.frdp)[which(sample_data(ps.frdp)$core=="GC1070"),17] <- 67
sample_data(ps.frdp)[which(sample_data(ps.frdp)$core=="GC1081"),17] <- 56
sample_data(ps.frdp)$smtzposition <- "above" # creates default value "above" for samples 
sample_data(ps.frdp)[which(sample_data(ps.frdp)$geochem_zone=="below"),18] <- "below" # overwrites as "below" for samples where geochem_zone = below
...
print(class.output) # list of classes by their relative abundance
...
Bubble plotting. Note that the Classes were all mixed up when I was plotting tax_reorder under aes(). Replotting by tax solves this, but I wanted to order Deltaproteobacteria and Methanomicrobia next to ANME and SRB. It's a small detail though.
```{r}
library(ggplot2)
...
On to importing the modeling data... hooo boy
```{r}
